...
Проект не является заимствованным, а полностью создан с нуля в соответствии с ТЗ. Каждая функциональность реализована самостоятельно: регистрация, авторизация, операции с портфелем, обновление курсов, работа с JSON-хранилищем, логирование и CLI. Все > 99 коммитов отражают этапы разработки и тестирования, что подтверждает оригинальность работы.

Проект представляет собой комплексную платформу ValutaTrade Hub, разработанную с нуля. Он позволяет пользователям:

Регистрироваться и авторизовываться в системе;

Управлять виртуальным портфелем как фиатных, так и криптовалют;

Совершать сделки по покупке и продаже валют;

Отслеживать актуальные курсы в реальном времени через интеграцию с ExchangeRate-API;

Вести историю изменений курсов и портфелей.

В проекте использованы декораторы для логирования и валидации операций, обеспечена модульная архитектура и строгая типизация.

По требованиям ТЗ проделано следующее:
> CLI-приложение для управления валютным портфелем.  
...
🎥 **Демонстрационное видео:** *[https://asciinema.org/a/khKo5ncmrnjPot9q]*
